...
export const gadiMastersSlice = createSlice({
    ...
    extraReducers: (builder => {
        ...
        builder.addCase(fetchGadiMastersAsync.fulfilled, (state, action) => {
            ...
            state.gadiNoList = action.payload.map((item: GadiMasters) => item.gadiNo).filter((gadiNo:string | null) => gadiNo !== null) as string[];

            //Comment By Durga//state.gadiNoList = action.payload.map((item: GadiMasters) => item.gadiNo);
            // if (Array.isArray(action.payload)) {
            //     gadiMastersAdapter.setAll(state, action.payload);
...
//             gadiMastersAdapter.setAll(state, action.payload);
//             state.gadiNoList = action.payload.map((item: GadiMasters) => item.gadiNo).filter((gadiNo:string | null) => gadiNo !== null) as string[];
//             state.allGadiMasters = action.payload.map((item: GadiMasters) => item);
//             state.status = 'idle';
//             state.gadiMastersLoaded = false;
